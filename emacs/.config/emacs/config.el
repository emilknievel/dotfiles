...
(defun my-theme-variants (base)
  "Return available themes whose names start with BASE."
...
common prefix (modus, catppuccin, ...)."
  (my-load-theme (intern (completing-read
                          (format "%s variant: " base)
                          (mapcar #'symbol-name (my-theme-variants base))
                          nil t))))
...
(use-package doom-themes
  ...
  ;; NOTE: These two lack a `lexical-binding' cookie upstream and warn on
  ;; load. Re-check and unskip with:
  ;;   grep -L lexical-binding ~/.config/emacs/elpaca/builds/doom-themes/*-theme.el
  (defconst my-doom-theme-exclusions '(doom-earl-grey doom-nord-aurora)
    "Doom themes excluded from `my-load-random-doom-theme'.")

  (defun my-load-random-doom-theme ()
    "Load a random doom theme."
    (interactive)
    ;; TODO: Drop this `seq-difference' and `my-doom-theme-exclusions' once
    ;; upstream ships the cookies.
    (let ((theme (seq-random-elt
                  (seq-difference (my-theme-variants "doom-")
                                  my-doom-theme-exclusions))))
      (my-load-theme theme)
  ...
  :general (my-leader-keys
             ...
             "t t z" 'my-zenburn
             "t t r" 'my-load-random-doom-theme))
...
(defun my-vulpea-select-work-note (&optional on-create)
  "Select a work note, creating it when missing.
ON-CREATE is called with the note when it had to be created."
  (let ((note (vulpea-select-from
               "Work note"
               (vulpea-db-query-by-tags-every (my-vulpea-work-note-tags)))))
    (if (vulpea-note-id note)
        note
      (let ((new (my-vulpea--create-work-note (vulpea-note-title note))))
        (when on-create (funcall on-create new))
        new))))
...
(defun my-vulpea-capture-work-task-target ()
  ...
  (vulpea-note-path
   (my-vulpea-select-work-note
    (lambda (note) (setq my-vulpea-capture--new-note note)))))
...
(defun my-vulpea--person-p (note)
  "Return non-nil when NOTE is a person note."
  (member "person" (vulpea-note-tags note)))
...
People are global: one note per person, linked from any domain."
  (interactive)
  (my-vulpea--find #'my-vulpea--person-p #'my-vulpea--create-person-note))

(defun my-vulpea-insert-person ()
  "Insert a link to a person note, creating it when missing."
  (interactive)
  (my-vulpea--insert #'my-vulpea--person-p #'my-vulpea--create-person-note))
...
(defun my-vulpea--project-p (note)
  "Return non-nil when NOTE is a project note."
...
(defun my-vulpea--work-project-p (note)
  ...
  (and (my-vulpea--project-p note)
       (my-vulpea-work-note-p note)))

(defun my-vulpea--create-project (title)
  "Create a personal project note titled TITLE, and return it."
