...
test("extractMemoryCandidatesFromMessages finds preference, project fact, and decision candidates", () => {
  const messages = [
    {
      role: "user",
      content: "Prefer minimal diffs. Ask before destructive commands.",
      timestamp: Date.now(),
    },
    {
      role: "assistant",
  ...
  assert.equal(
    result.some(
      (item) =>
        item.kind === "preference" && /Prefer minimal diffs/i.test(item.text),
    ),
    true,
  );
  assert.equal(
    result.some(
      (item) =>
        item.kind === "project_fact" && /This repo uses pnpm/i.test(item.text),
    ),
    true,
  );
  assert.equal(
    result.some(
      (item) =>
        item.kind === "decision" &&
        /Keep schemas in src\/schema/i.test(item.text),
    ),
    true,
  );
});
  ...
  const messages = [
    {
      ...
      content: "Old memory block",
      display: false,
      timestamp: Date.now(),
    },
    {
      role: "user",
      content: "Prefer minimal diffs. Prefer minimal diffs.",
      timestamp: Date.now(),
    },
  ];
  const result = extractMemoryCandidatesFromMessages(messages);
