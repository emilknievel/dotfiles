...
test("extractObservedCandidates learns conservative bash facts from safe commands", () => {
  ...
  assert.equal(
    result.some((item) => item.text === "This repo uses pnpm"),
    true,
  );
  assert.equal(
    result.every((item) => item.confidence < 0.5),
    true,
  );
});
...
test("extractObservedCandidates ignores install and compound commands", () => {
  assert.deepEqual(
    extractObservedCandidates("bash", { command: "pnpm install" }),
    [],
  );
  assert.deepEqual(
    extractObservedCandidates("bash", { command: "pnpm test && echo done" }),
    [],
  );
});

test("mergeObservedCandidates respects candidate-specific thresholds", () => {
  const pnpmObserved = extractObservedCandidates("bash", {
    command: "pnpm test",
  });
  ...
  assert.equal(
    merged.accepted.some((item) => item.text === "This repo uses pnpm"),
    true,
  );

  const zodObserved = extractObservedCandidates("bash", {
    command: "pnpm test zod",
  });
  let zodMerged = mergeObservedCandidates(new Map(), zodObserved);
  assert.equal(
    zodMerged.accepted.some((item) => item.observationKey === "bash:zod"),
    false,
  );
